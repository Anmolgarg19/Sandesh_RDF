...
function demolink()
{ var name='www.youtube.com/watch?v=pt1j2k1M97o';
  var url = name.replace(" ","_");
  url = "http://"+url;
...
function indexinfo(theme)
{
	document.getElementById("header_names").innerHTML = "" ;
	document.getElementById("inputTheme").value=theme;
	document.getElementById("searchedname").innerHTML = theme.split('/').pop() ;
	document.getElementById("vocabulary").innerHTML = theme;
	document.getElementById('schemabutton').style.display = "block";
	...
	if($(obj).is(":checked")){
			...
			$.ajax({
				...
				for(var key in new_val[0]){
					...
					for(var key2 in value){
						...
						//a.onclick = 	
						my_url = "{{ url_for('goindexafterclick', theme="+value1[key2]+") }}"
						//console.log(value1[key2] + "is theme");
						///var req_url = url_for('goindexafterclick', theme = value1[key2]);
						a.setAttribute("title",value1[key2]);
						a.setAttribute("href", "");
						//a.setAttribute("href", "/goindexafterclick/"+value1[key2]+"");
						//a.setAttribute("onClick","getdata("+value1[key2]+")");
						a.appendChild(textvalue)
						td.appendChild(a);
						//td.onClick = "getdata("+value1[key2]+")";
						tr2.appendChild(td);
						if(temp%size == 0){
        ...
        "sDom": '<"row view-filter"<"col-sm-12"<"pull-left"f><"pull-right"l><"clearfix">>>t<"row view-pager"<"col-sm-12"<"text-enter"ip>>>'
				...
				for (var i = 0; i < themename.length; i++)
				{
					...
					var header_uri= temp.split("|")[1];
					var prop_name= temp.split("|")[2];
					var anchoringcolumn_uri=temp.split("|")[3].split("\"")[0];
					var anchoringcolumn_name=anchoringcolumn_uri.split(".")[anchoringcolumn_uri.split(".").length-1];
					html+=col_name;
					...
					myCircle.setAttributeNS(null,"cy",x2.top-140);
...
//myCircle.setAttribute("onclick","colorchange(\""+col_name+"\",\""+anchoringcolumn_temp+"\",\""+prop_name+"\",\""+anchoringcolumn_uri+"\",\""+header_uri+"\")");			
			...
			myCircle.setAttribute("onclick","colorchange(\""+col_name+"\",\""+anchoringcolumn_name+"\",\""+prop_name+"\",\""+anchoringcolumn_uri+"\",\""+header_uri+"\")");
